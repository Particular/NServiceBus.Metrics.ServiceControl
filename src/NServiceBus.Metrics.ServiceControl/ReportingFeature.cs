...
{
    public ReportingFeature() => DependsOn<MetricsFeature>();
    ...
    protected override void Setup(FeatureConfigurationContext context)
    {
        ...
        var reportingOptions = context.Settings.Get<ReportingOptions>();

        SetUpServiceControlReporting(context, reportingOptions, endpointName, metrics);

        context.Settings.AddStartupDiagnosticsSection("Manifest-MonitoringQueue", reportingOptions.ServiceControlMetricsAddress);
    }
    ...
    void SetUpServiceControlReporting(FeatureConfigurationContext context, ReportingOptions options, string endpointName, Dictionary<string, Tuple<RingBuffer, TaggedLongValueWriterV1>> durations)
    {
        Dictionary<string, string> BuildBaseHeaders(IServiceProvider b)
        {
            ...
            if (!string.IsNullOrEmpty(options.EndpointInstanceIdOverride))
            {
                headers.Add(MetricHeaders.MetricInstanceId, options.EndpointInstanceIdOverride);
            }
        ...
        if (!string.IsNullOrEmpty(options.EndpointInstanceIdOverride))
        {
            context.Services.AddSingleton<IMutateOutgoingMessages>(new MetricsIdAttachingMutator(options.EndpointInstanceIdOverride));
        }
    }
    ...
    class ServiceControlRawDataReporting(IServiceProvider builder, ReportingOptions options, Dictionary<string, string> headers, Dictionary<string, Tuple<RingBuffer, TaggedLongValueWriterV1>> metrics)
        : FeatureStartupTask
    {
    ...
    class MetricsIdAttachingMutator(string instanceId) : IMutateOutgoingMessages
    {
